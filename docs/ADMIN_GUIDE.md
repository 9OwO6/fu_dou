...
7. 管理墙支持按编号/名称搜索、按状态筛选和批量恢复、售完、归档。展开“编辑图片、名称、说明、价格和标签”，可在同一卡片中修正全部可选内容。
8. 图片编辑区支持一次追加多张图片、逐张替换或移除，并可将任意一张设为封面。每件展示商品最多 10 张、至少保留 1 张；替换会保留原来的展示位置，移除后其余图片会自动连续排序。
9. 图片上传或文件清理失败时留意编辑区提示；系统会尽量撤销新文件或恢复原图片资料。若提示“请停止继续编辑并联系技术人员”，不要继续重复操作。
10. 快速上新不会生成正式商品、库存、购物车或订单请求。需要在线接受订单请求时，必须在正式“商品管理”中建立商品、规格、SKU、价格和库存。
11. 发布前在“选择本批陈列效果”中选择：日常稳定上新使用“阳光陈列架”，希望更有小店个性时使用“快乐手账拼贴”，少量限定或重点宣传使用“今日主推”。可点任一商品的“设为主推”；不指定时默认第一件。
12. 已发布批次可在 `/admin/quick-listings` 的“批次陈列设置”中重新选择效果和主推商品。该设置作用于整批新品；较早批次在公开页会自动回归统一商品墙。
