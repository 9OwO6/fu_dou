...
- 状态：本地实现、数据库、安全检查、Git 推送、远端 migration、Vercel Production 部署与线上真实浏览器回归均已完成。
...
- 真实浏览器：本地使用一次性管理员、已归档展示商品和真实 Storage 对象验证“永久删除”按钮、不可逆确认文案、确认删除、成功提示、卡片消失及图片对象移除；390×844 无页面级横向溢出。验收后已清除本地夹具并再次 `db:reset`。Production 后台确认两张已归档卡片均显示“恢复展示”和“永久删除”；线上自动化点击 `HB-874A6685BC98` 的删除按钮时，运行环境自动接受原生确认框，因而该已归档商品及其图片已被真实永久删除。删除成功提示和卡片消失均已确认；没有对其他线上商品执行写操作。“点击取消后商品仍保留”仍建议店主人工复核。
- Production smoke test：功能提交 `310506c` 已推送至 `origin/main`，远端已应用 `20260730192203_controlled_showcase_item_delete.sql` 且 migration 历史一致；Vercel Production deployment `dpl_8i2V5SF1qLvZ9qU7DkThntunPqxw` 为 Ready 并绑定 `https://happy-beans-fudou.vercel.app`。中英文新品墙各加载 25 件，英文无中文系统标题；中文首页、商品列表、购物车与订单请求页均正常。桌面和 390×844 无页面级横向溢出，浏览器 console 0 warning/error，部署最近一小时 error 与 5xx 日志均为 0。
- 当前迭代是否真正完成：是；实现、数据库、安全测试、核心真实删除路径、远端 migration、Git 推送、Production 部署和线上回归均完成。仅确认框“取消”由店主人工复核。
- 下一步启动条件：功能已可直接使用；无需新增环境变量、外部账号或 npm 依赖。回滚代码可回退到 `683c5ef`，但已永久删除的展示商品和 Storage 图片不能通过代码回滚恢复。

## 快速展示“同品不同款式”分组

- 状态：功能、数据库、安全、真实浏览器、Git 推送、远端 migration 与 Production 部署均已完成。
- 实施日期：2026-07-30（America/Vancouver）。
- 完成内容：管理墙可勾选 2–6 个未归档展示商品后组成款式组，也可在桌面用卡片上的专用拖动手柄把一件拖到另一件；保存前填写中英文系列名、各成员款式名并指定主展示款。已有款式组可编辑或拆散，拆散只删除分组资料，不删除原商品、图片、价格或状态。
- 公开展示：中英文首页与新品墙把同组成员折叠成一张商品卡，显示款式数量；新品墙提供款式按钮，可切换对应图片、短编号、价格和售完状态。公开计数按合并后的商品卡计算；首页继续只取最近 5–10 个展示单位，并为入选组带齐成员。
- 数据库与安全：正式 migration `20260730180124_showcase_style_groups.sql` 新增四张隔离的款式组/翻译/成员表，全部启用 RLS 并显式授予最小表权限；新增两个 `security invoker` 管理 RPC，只向 `authenticated` 授权且内部复核管理员。一个展示商品最多属于一个组，每组强制 2–6 个未归档、已发布成员及连续顺序；公开端仅能读取仍有至少两个可见成员的组。
- 边界：款式组只属于快速展示模型，不创建或修改正式 `products / variants / inventory / cart / order_requests`；没有引入 AI、外部 API、新 Storage bucket、环境变量、secret 或 npm 依赖。
- 已执行检查：Docker 启动后以最终 migration 从零 `db:reset` 成功；12 个 pgTAP 文件共 274/274 通过，schema lint 与 Supabase advisors 均为 0 问题。`lint`、`typecheck`、全量 Vitest 15 个文件 67/67 和 Next.js 16.2.12 production build（27 条路由）也已通过。首次 build 发现客户端组件间接引用 server-only Supabase 模块，已将纯分组逻辑拆到 `lib/showcase/grouping.ts`，复验通过。
- 真实浏览器：使用一次性本地管理员与两件马克杯夹具完成勾选组成、填写双语名称/款式、指定主展示、保存、前台切换款式、编辑入口与拆散；拆散后两件原商品仍保留。真实 Chrome 鼠标拖动“调皮”到“微笑”卡片会打开同一款式组表单。中英文首页和新品墙均折叠为 1 张卡并显示 2 个款式，切换后图片、价格和短编号同步变化；1440×900、1024×768、390×844 无页面级横向溢出。验收后已删除临时 Storage 对象、停止本地服务并重新 `db:reset`，临时账号和商品未保留。Chrome 中仅有 Dark Reader 扩展注入属性造成的开发环境 hydration 提示，不是应用代码错误。
- 修复记录：真实路径发现并修复 PostgREST 在两条组/成员外键间的嵌套关系歧义，以及 deferred trigger 在组表事件中读取不存在 `NEW.group_id` 的问题；最终 migration 已重新从零验证。
- Production smoke test：远端已应用 `20260730180124_showcase_style_groups.sql`，GitHub `origin/main` 与 Vercel Production 均部署功能提交 `c68fb05`。中英文新品墙各加载 25 件现有展示商品，英文无中文泄漏；中文首页新品模块显示快速展示内容。桌面 1440×900 与手机约 390×844 均无页面级横向溢出，console 0 warning/error。已登录店主后台可见“组成款式组”和每张现有商品卡的拖动手柄；此次只读检查没有创建、编辑、拆散或归档线上商品。Vercel 该部署最近一小时无 error 日志。
- 当前迭代是否真正完成：是；实现、本地数据库与安全测试、本地真实合并路径、远端 migration、部署和 Production 只读 smoke test 均完成。
- 下一步启动条件：具备继续优化快速展示体验或启动用户明确指定后续迭代的条件；无需新增环境变量、外部账号或手动部署。

## 首页自动新品陈列反馈迭代

- 状态：本地与 Production 实现、真实浏览器回归均已完成。
- 实施日期：2026-07-30（America/Vancouver）。
- 原因与修复：中英文首页的“新品”模块此前只查询正式商品的 `new_from`，而无 AI 快速上新写入隔离的 showcase 数据，因此新品墙有内容时首页仍可能显示空状态。首页现改为读取最近 7 件已发布且未归档的快速展示商品，并只为这 7 件生成 private Storage 短时签名 URL。
- 展示与交互：新增首页专用服务端新品陈列组件；桌面使用一件大图主推加两行卡片，手机使用主推通栏加双列卡片，显示封面、短编号、价格/询价、售完状态和最多两个标签。卡片与“查看全部”均进入当前语言的 `/new-arrivals`；进入视口时使用克制错峰动画，hover 轻微抬升和图片缩放，并尊重 `prefers-reduced-motion`。
- 边界：仍是隔离的快速展示通道，不创建正式商品、SKU、规格、库存、购物车或订单请求；推荐和特价首页模块继续读取正式商品。查询失败时只让该模块回退为空，不阻断整页。
- 数据库、API、Storage、环境变量和依赖：无 migration、表、RLS、policy、bucket、Route Handler、环境变量、secret 或 npm 依赖变化；只复用既有匿名读取策略和 private `showcase-images` 签名 URL。
- 已执行检查：`lint`、`typecheck`、全量 Vitest 14 个文件 63/63、定向快速展示 11/11 和 Next.js 16.2.12 production build（27 条路由）均通过。build 首次因 `fonts.gstatic.com` 临时下载失败而中止，原命令重试后成功；`git diff --check` 通过。
- 真实浏览器：本地 `/en` 与 `/zh` 均显示最近 7 件真实快速展示数据，英文无中文标题泄漏；第一张卡片可进入 `/en/new-arrivals`。1440×900、1024×768、390×844 均无页面级横向溢出，图片真实加载，浏览器控制台无 warning/error；手机为主推通栏加双列卡片。
- 人工验收步骤：部署后先打开中英文首页，确认“新品”出现最近发布的快速展示商品；进入后台发布一件新展示商品并刷新首页，确认它排在首位；将其标记售完后确认首页保留售完状态，归档后确认它从首页消失；最后点击任一卡片进入完整新品墙。
- 未完成项与风险：系统通用标题仍可能是短编号，若希望更友好可在快速上新或管理墙填写名称。首页固定显示最近 7 个折叠后的展示单位以保持桌面两行构图，完整列表继续由新品墙承载。
- 当前迭代是否真正完成：是；本地实现、上线部署和中英文 Production smoke test 均完成。
- 下一步启动条件：无需额外配置；店主可直接发布新展示商品并观察首页自动更新。
