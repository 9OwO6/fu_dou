...
- `/[locale]` 首页的受控“新品”模块复用同一匿名读取边界，但只按创建时间读取最近 7 件已发布、未归档的快速展示商品并只签名这些封面；查询失败只回退该模块。首页使用独立服务端陈列组件，卡片统一跳转完整新品墙，不接入正式商品、购物车或订单请求。
...
- “同品不同款式”继续位于该隔离展示通道：`showcase_style_groups` 保存主展示成员，组名和成员款式名分别使用中英文 translation 表；`showcase_style_group_items` 只引用既有展示商品，不复制图片、价格或状态。每件最多属于一个组，每组限制 2–6 件。
- 管理墙的勾选合并和桌面专用拖动手柄最终都进入同一个确认对话框，再由 Server Action 调用 `admin_save_showcase_style_group`。RPC 使用 `security invoker`，复核管理员、已发布/未归档成员、去重、连续顺序、唯一归组和主展示成员；`admin_dissolve_showcase_style_group` 只级联删除分组资料并保留展示商品。
- 匿名 RLS 仅在一个组仍有至少两个已发布且未归档成员时暴露组和成员资料。前台纯分组层按组折叠卡片，并以所选成员驱动图片、短编号、价格和状态；首页最近项查询会多取候选后按 5–10 个折叠展示单位截断，确保入选组的款式可完整切换。
- `admin_delete_showcase_item` 是快速展示商品唯一永久删除入口，只接受已归档商品，并在同一事务内拆散其款式组、级联删除翻译/图片元数据/标签关系/展台成员及写入审计。Server Action 取得 RPC 返回的全部路径后必须通过 Storage API 删除真实对象；数据库成功但文件清理失败时返回明确警告，不把已删除商品恢复成公开内容。
